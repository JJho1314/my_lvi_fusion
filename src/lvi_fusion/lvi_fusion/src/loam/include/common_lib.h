...
static const Eigen::Vector3d Lidar_offset_to_IMU(0.04165, 0.02326, -0.0284); // Avia  til

struct Pose6D // 位姿+时间戳
{
...
struct Camera_Lidar_queue
{
    ...
    int m_if_acc_mul_G = 0;  // 是否需要将单位从G转化成m/s^2
    ...
    Eigen::Vector3d g_noise_cov_acc; // IMU_acc_cov
    ...
    std::deque<sensor_msgs::PointCloud2::ConstPtr> *m_liar_frame_buf = nullptr;   // 指向原始点云lidar_buffer
    ...
    int camera_in(const double &in_time)
    {
     ...
     // 原始buff队列中的第一帧的的时间戳（当前要处理的lidar的时间）
    double get_lidar_front_time()
    {
        if (m_liar_frame_buf != nullptr && m_liar_frame_buf->size())
        {
        
            m_last_lidar_time = m_liar_frame_buf->front()->header.stamp.toSec() + 0.1; // 因为lidar是一个scan周期都在扫描，而时间戳记录的是第一个点的时间，所以要+0.1
    ...
    {
        // m_if_have_lidar_data = 1;
        double cam_last_time = get_camera_front_time(); // 上一次处理cam的时间
        double lidar_last_time = get_lidar_front_time(); // 原始buff队列中的第一帧的的时间戳（当前要处理的lidar的时间）
        if (m_if_have_camera_data == 0) // 没有第一帧相机
        {
            return true;
        }
        ...
        // 现在cam时间更早，等处理完cam再处理lidar（防止一直处理lidar）
        if (lidar_last_time > cam_last_time)
        {
...
struct MeasureGroup // Lidar data and imu dates for the curent process （the PC of one scan ）
{
    MeasureGroup()
    {
        this->lidar.reset(new PointCloudXYZINormal());
    };
    double lidar_beg_time; // 一个scan的开始时间
    double lidar_end_time; // 一个scan的结束时间
    PointCloudXYZINormal::Ptr lidar;
    std::deque<sensor_msgs::Imu::ConstPtr> imu; // 从上一个scan结束到这一个scan结束的IMU数据
};
...
public:
    ...
    Eigen::Matrix<double, DIM_OF_STATES, DIM_OF_STATES> cov; // states covariance  状态方差P    
    double last_update_time = 0;
    double          td_ext_i2c;                              // 时间延迟
    StatesGroup()
    {
