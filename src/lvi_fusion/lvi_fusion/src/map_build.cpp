...
Eigen::Vector3d currOdom;  //add by dawn
...
void saveOptimizedVerticesTUMformat(gtsam::Values _estimates, std::string _filename)
{
    ...
    for(int i=0; i<keyframePosesUpdated.size();++i ) {
        ...
        stream << setprecision(6) << keyframeTimes[i] << setprecision(7) << " " << t.x() << " "<< t.y() << " " << t.z()<< " " 
...
void gpsHandler(const sensor_msgs::NavSatFix::ConstPtr &gpsMsg)
{
    ...
    //     mBuf.unlock();
    // }
    gps_current.latitude = gpsMsg->latitude;
    gps_current.longitude = gpsMsg->longitude;
    gps_current.altitude = gpsMsg->altitude;
    gps_current.header = gpsMsg->header;
...
void gpsOdomHandler(const nav_msgs::Odometry::ConstPtr &gpsMsg)
{
    ...
    if(optimization_with_GPS&&(gpsMsg->pose.covariance[0]<0.1&&gpsMsg->pose.covariance[7]<0.1)){
        ...
        mBuf.lock();
        // gpsBuf.push(make_pair(Eigen::Vector3d(Pwl.x(),Pwl.y(),Pwl.z()),gpsMsg->header.stamp.toSec()));
        currOdom = Pwl;   //add by dawn
...
void addOdomFactor(int& prev_node_idx, int& curr_node_idx)
{
    if( ! gtSAMgraphMade /* prior node */) {  // 第一帧先验
       ...
       
        {
            ...
            // gps factor 
            if(1) {   // 当前帧有对应的GPS
                ROS_INFO("%.2f,%.2f,%.2f",currGPS.x(), currGPS.y(), currGPS.z());  //add by dawn
                ...
                if(evalFormat =="kitti"){
                    ...
                    << col1.z() << " " << col2.z() << " " << col3.z() << " " << currGPS.z() << std::endl;
                        ...
                        Eigen::Matrix3d tmpR(currGPSQ);         
                                    ...
                                    // << gps_current.longitude << " "<< gps_current.latitude << " "<< gps_current.altitude 
                    // ROS_INFO("Lon:%f, Lat:%f",gps_current.longitude ,gps_current.latitude);
                     groud_truth_stream << tmpR(0,0) << " " << tmpR(0,1) << " " << tmpR(0,2) << " " << gps_current.longitude << " "
                    << tmpR(1,0) << " " << tmpR(1,1) << " " << tmpR(1,2) << " " << gps_current.latitude << " "
                    << tmpR(2,0) << " " << tmpR(2,1) << " " << tmpR(2,2) << " " << gps_current.altitude << std::endl;
                    }
...
void process_pg()
{
    ...
    while(ros::ok())
    {
		while ( !odometryBuf.empty() && !fullResBuf.empty() )  // 
        {
            ...
            // timeLaser = ros::Time::now().toSec(); // 点云时间
            ...
            tf::Quaternion quat(currGPSQ.w(),currGPSQ.x(),currGPSQ.y(),currGPSQ.z());

    
            double roll, pitch, yaw;//定义存储r\p\y的容器
            tf::Matrix3x3(quat).getRPY(roll, pitch, yaw);//进行转换

            // pose_curr.x = currOdom.x();    //add by dawn
            // pose_curr.y = currOdom.y();    //add by dawn
            // pose_curr.z = currOdom.z();    //add by dawn
            // pose_curr.roll = roll;
            // pose_curr.pitch = pitch;
            // pose_curr.yaw = yaw;
