...
class IMUFactor : public ceres::SizedCostFunction<15, 7, 9, 7, 9>
{
  public:
    IMUFactor() = delete;
    // TODO 这里把引用&去掉了，记得改回来
    IMUFactor(ImuProcess _pre_integration):pre_integration(_pre_integration)
    {
    }
    /**
...
//delta_q = Qi.inverse() * Qj;
        ...
        */
        ...
        // cout << "imu factor residual:" << residual <<endl;
        ...
        */
        ...
        // cout << "IMU的残差是："<<residual<< "信息矩阵是"<< sqrt_info<<endl;
        residual = sqrt_info * residual;
    //    cout << "IMU的残差是："<<residual<<endl;
        // 计算残差对于优化变量的Jacobian
        if (jacobians)
        {
            // 预积分结束时刻误差相对于预积分起始时刻误差的微分，用于修正预积分增量
            double sum_dt = pre_integration.dtime;
...
///                ROS_BREAK();
            ...
            if (jacobians[0])
            {
                ...
                jacobian_pose_i.block<3, 3>(O_P, O_R) = Utility::skewSymmetric(Qi.inverse() * (0.5 * vG * sum_dt * sum_dt + Pj - Pi - Vi * sum_dt));

#if 0
            jacobian_pose_i.block<3, 3>(O_R, O_R) = -(Qj.inverse() * Qi).toRotationMatrix();
#else
                Eigen::Quaterniond corrected_delta_q = pre_integration.dq * Utility::deltaQ(dq_dbg * (Bgi - pre_integration.linearized_bg));
                jacobian_pose_i.block<3, 3>(O_R, O_R) = -(Utility::Qleft(Qj.inverse() * Qi) * Utility::Qright(corrected_delta_q)).bottomRightCorner<3, 3>();
#endif

                jacobian_pose_i.block<3, 3>(O_V, O_R) = Utility::skewSymmetric(Qi.inverse() * (vG * sum_dt + Vj - Vi));
            ...
            jacobian_speedbias_i.block<3, 3>(O_R, O_BG - O_V) = -dq_dbg;
                ...
                jacobian_speedbias_i.block<3, 3>(O_R, O_BG - O_V) = -Utility::Qleft(Qj.inverse() * Qi * pre_integration.dq).bottomRightCorner<3, 3>() * dq_dbg;
#endif
